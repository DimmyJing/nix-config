...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
        ...
        lua_ls = {
          ...
          settings = {
            Lua = {
              ...
              workspace = {
                library = { '/Users/jimmy/Workspace/sandbox/computercraft/computercraft.lua' },
              },
            },
          },
        },
      ...
      for server_name, server in pairs(servers) do
        local has_ensure = false
        for _, ensure in ipairs(ensure_installed) do
          if server_name == ensure then
            has_ensure = true
            break
          end
        end
        if not has_ensure then
          if need_install[server_name] then
            require('lspconfig.configs')[server_name] = { default_config = server }
          end
          server.capabilities = vim.tbl_deep_extend('force', {}, capabilities, server.capabilities or {})
          lspconfig[server_name].setup(server)
        end
  ...
  -- place them in the current locations.
